...
def generate_batch(data, batch_size, skip_window):
    ...
    Batch is a vector of shape (batch_size, 2*skip_window), with each entry for the batch containing all the context words, 
    with the corresponding label being the word in the middle of the context
    """
    global data_index
    ...
    # span is size of the context
    span = 2 * skip_window + 1  # [ skip_window target skip_window ]
    
    # buffer is a queue of words
    # size of the buffer is limited to size of the context
    ...
    for i in range(batch_size):
        for j in range(2 * skip_window):
            # assign words who are not at the middle (skip_window) of the context to the batch
            if (j < skip_window):
                batch[i, j] = buffer[j]
            else:
                batch[i, j] = buffer[j + 1]
            
        # assign the target to the labels  
        # the value of skip_window is right the middle index of the context
